...
export default function Footer() {
  return (
    <footer className="mt-20 border-t border-[var(--border)] px-4 pb-10 pt-10 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl rounded-[2rem] bg-[rgba(255,250,240,0.74)] p-8 shadow-[0_30px_80px_rgba(15,26,42,0.08)] backdrop-blur md:p-10">
              ...
              <div className="mt-5 space-y-3">
                {items.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="block text-sm text-[var(--ink)] transition hover:text-[var(--brand)]"
                  >
                    {item.label}
                  </Link>
                ))}
        ...
        <div className="mt-10 flex flex-col gap-3 border-t border-[var(--border)] pt-6 text-sm text-[var(--ink-soft)] md:flex-row md:items-center md:justify-between">
