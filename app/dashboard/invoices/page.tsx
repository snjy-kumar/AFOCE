...
export default function InvoicesPage() {
  ...
  const filtered = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesQuery = lower.length === 0 || item.id.toLowerCase().includes(lower);
  ...
  const handleCreated = () => {
    setShowCreate(false);
    fetchInvoices().then(setItems);
  };

  const stats = [
    { label: "Total Invoices", value: items.length, amount: "Rs. 0", color: "text-[var(--brand)]", bg: "bg-[var(--brand)]/10" },
    { label: "Paid", value: items.filter((i) => i.status === "paid").length, amount: "Rs. 0", color: "text-[var(--brand-2)]", bg: "bg-[var(--brand-2)]/10" },
    { label: "Overdue", value: items.filter((i) => i.status === "overdue").length, amount: "Rs. 0", color: "text-[var(--danger)]", bg: "bg-[var(--danger)]/10" },
    { label: "Pending", value: items.filter((i) => i.status === "pending").length, amount: "Rs. 0", color: "text-[var(--accent)]", bg: "bg-[var(--accent)]/10" },
  ];

  return (
    <div className="space-y-6">
      {showCreate && <InvoiceModal onClose={() => setShowCreate(false)} onCreated={handleCreated} />}
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-white">
            ...
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--bg-elevated)]/50">
                <th className="px-4 py-3">
                  <input type="checkbox" checked={selectedItems.size === filtered.length && filtered.length > 0} onChange={toggleSelectAll} className="h-4 w-4 rounded border-[var(--border)]" />
                </th>
              ...
              {filtered.map((invoice) => (
                <tr key={invoice.id} className="transition hover:bg-[var(--bg-elevated)]">
                  <td className="px-4 py-4">
                    <input type="checkbox" checked={selectedItems.has(invoice.id)} onChange={() => toggleSelect(invoice.id)} className="h-4 w-4 rounded border-[var(--border)]" />
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4">
                    <div className="font-medium text-[var(--ink)]">{(invoice as unknown as Record<string, unknown>).client_name as string || invoice.client_id}</div>
                    <div className="text-xs text-[var(--ink-soft)]">PAN: {(invoice as unknown as Record<string, unknown>).client_pan as string || ""}</div>
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${
                      invoice.status === "paid" ? "bg-[var(--brand-2)]/10 text-[var(--brand-2)]"
                      : invoice.status === "overdue" ? "bg-[var(--danger)]/10 text-[var(--danger)]"
                      : invoice.status === "pending" ? "bg-[var(--accent)]/10 text-[var(--accent)]"
        ...
        <div className="flex items-center justify-between border-t border-[var(--border)] px-4 py-3">
          <div className="text-sm text-[var(--ink-soft)]">Showing {filtered.length} of {items.length} invoices</div>
          <div className="flex gap-1">
            <button type="button" className="rounded-lg border border-[var(--border)] bg-white px-3 py-1.5 text-sm font-medium text-[var(--ink-soft)] transition hover:bg-[var(--bg-elevated)]">Previous</button>
            <button type="button" className="rounded-lg bg-[var(--brand)] px-3 py-1.5 text-sm font-medium text-white">1</button>
            <button type="button" className="rounded-lg border border-[var(--border)] bg-white px-3 py-1.5 text-sm font-medium text-[var(--ink-soft)] transition hover:bg-[var(--bg-elevated)]">Next</button>
          </div>
        </div>
      </div>
    </div>
